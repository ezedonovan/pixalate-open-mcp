...
def get_enrichment_mobile_app(
        request: EnrichmentMobileRequest
) -> Dict:
...
def get_enrichment_ctv_app(
        request: EnrichmentCTVRequest
) -> Dict:
    return _handle_enrichment_request(
        ...
        params=dict(
            device=request.device,
            region=request.region,
            includeSpoofing=request.includeSpoofing
        )
    )
...
def get_enrichment_domains(
        request: EnrichmentDomainRequest
) -> Dict:
    return _handle_enrichment_request(
...
def _handle_enrichment_request(
        url,
        app_or_domain_ids: List[str],
        column_name: str,
        params: dict
) -> Dict:
    if len(app_or_domain_ids) > 1:
        download_url = _handle_csv_upload(
            url=url,
            column_name=column_name,
            data=app_or_domain_ids,
            params=params
        )
